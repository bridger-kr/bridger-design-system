...
1. Figma 데스크톱 → Plugins → Development → **Import plugin from manifest…** → `packages/figma-plugin/plugin/manifest.json`
...
현재 `manifest.json`에는 Figma가 발급한 실제 플러그인 ID
`"1645391438739186339"`가 들어 있습니다. 재퍼블리시할 때도 이 ID를 유지해야
기존 Figma Community 플러그인 항목에 업데이트됩니다.
