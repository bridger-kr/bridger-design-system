...
export function Slider({
  label, min = 0, max = 100, step = 1, value, defaultValue, onChange, unit = '', hint, id, style,
}) {
  ...
  const onKey = (e) => {
    ...
    else if (e.key === 'ArrowLeft' || e.key === 'ArrowDown') { e.preventDefault(); set(v - step); }
  };
  ...
  return (
    <div style={{ display: 'grid', gap: 9, ...style }}>
      ...
      >
        <div style={{ position: 'absolute', left: 0, right: 0, height: 4, borderRadius: 2, background: 'var(--dt-surface-sunken)', boxShadow: 'inset 0 0 0 1px var(--dt-border-strong)' }} />
        <div style={{ position: 'absolute', left: 0, width: `${pct}%`, height: 4, borderRadius: 2, background: 'var(--dt-accent)' }} />
        <div style={{
          position: 'absolute', left: `${pct}%`, transform: 'translateX(-50%)',
          width: 16, height: 16, borderRadius: 'var(--dt-radius-sm)', background: 'var(--dt-surface)',
